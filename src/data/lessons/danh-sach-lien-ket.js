export default {
  ...
  project: {
    ...
    needs: [
      'Nút, con trỏ sau, và nút giả ở phần Lý thuyết bài này — nhất là vì sao nút giả làm erase_after gọn hẳn.',
      'Kỹ thuật hai con trỏ nhanh chậm, dùng cho cả tìm nút giữa lẫn phát hiện chu trình.',
      'C++: con trỏ, new/delete, và vì sao huỷ danh sách phải dùng vòng lặp chứ không dùng đệ quy.',
      'Vec và Str bạn đã viết ở bài Mảng, chuỗi và mảng động. Dự án này gộp chung với chúng thành thư viện core.',
      'Makefile mức tối thiểu: một đích all, một đích test. Chỉ cần đúng hai đích đó.',
    ],
    output: 'Thư mục core/ hoàn chỉnh gồm Vec, Str, List, một Makefile chạy được bằng make test, và một chương trình so List với Vec ở hai phép việc trái ngược nhau.',
    outputSample: `$ make test
[ok] xoa nut giua khong ro ri
[ok] phat hien dung chu trinh
[ok] huy 1000000 nut khong ro ri
3/3 ca xanh

$ ./demo_list
phep viec | Vec ms | List ms | ai thang
cong tong |   0.42 |    4.81 | Vec, nho cache
chen dau  | 210.35 |    0.02 | List`,
    start: [
      'Viết struct Nut { int giaTri; Nut* sau; } và một hàm push_front. Thêm ba số rồi duyệt in ra. Chưa cần template, chưa cần nút giả.',
      'Thêm nút giả đứng trước dữ liệu thật, viết lại push_front cho quen. Đây là bước làm mọi thứ sau đó dễ đi.',
      'Viết erase_after rồi kiểm bằng mắt: xoá nút giữa, in lại cả danh sách.',
      'Viết hàm huỷ danh sách bằng vòng lặp, kiểm với một triệu nút. Nếu bạn lỡ viết bằng đệ quy thì đây đúng là lúc nó tràn ngăn xếp cho bạn xem tận mắt.',
      'Thêm hai con trỏ nhanh chậm: làm tìm nút giữa trước vì nó dễ hơn, phát hiện chu trình sau.',
      'Cuối cùng mới đổi sang template, gộp vào core/ cùng Vec và Str, viết Makefile, rồi đo.',
    ],
    must: [
      ...
      {
        dat: 'push_back là O(1), không phải O(n).',
        kiem: 'Đo push_back 100000 lần: tổng thời gian tăng tuyến tính theo số lần, không tăng theo bình phương. Nếu nó bốc lên thì bạn đang duyệt tới đuôi mỗi lần thay vì giữ con trỏ đuôi.',
      },
      {
        dat: 'Xoá nút giữa đúng và không rò rỉ.',
        kiem: 'Tạo danh sách 5 nút, erase_after nút thứ 2, in lại: còn đúng 4 nút theo đúng thứ tự. Chạy dưới -fsanitize=address báo 0 byte rò rỉ.',
      },
      {
        dat: 'Huỷ danh sách một triệu nút không sập.',
        kiem: 'Tạo và huỷ danh sách 1000000 nút. Sập ở đây nghĩa là bạn huỷ bằng đệ quy và đã tràn ngăn xếp — phải đổi sang vòng lặp.',
      },
      {
        dat: 'Phát hiện chu trình đúng ở cả hai chiều.',
        kiem: 'Chạy trên một danh sách thẳng (phải trả về không có chu trình) và một danh sách được nối đuôi về giữa (phải trả về có). Sai một trong hai ca là chưa đạt.',
      },
      {
        dat: 'Thư viện core biên dịch và kiểm được bằng một lệnh.',
        kiem: 'make test từ thư mục core/ — biên dịch cả Vec, Str, List rồi in ra số ca xanh, tối thiểu 3/3.',
      },
      {
        dat: 'Đo được và giải thích được chỗ List thua Vec.',
        kiem: 'Bảng so sánh cho thấy List thắng rõ ở chèn đầu và thua rõ ở cộng tổng. Viết một câu giải thích khoảng cách ở cộng tổng bằng mô hình cache đã học ở bài Độ phức tạp.',
      },
    ],
    traps: [
