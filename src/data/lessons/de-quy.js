export default {
  ...
  project: {
    ...
    why: 'Mọi công cụ phân tích hiệu năng thật đều đếm số lời gọi hàm chứ không chỉ đo thời gian, vì số lời gọi không phụ thuộc máy đang chạy nhanh hay chậm lúc đó. Module này gắn vào bench của MVP Chương 1 và được dùng lại ở Chương 2 để kiểm merge sort có đúng O(n log n) như lý thuyết hứa.',
    input: 'Không đọc dữ liệu ngoài; nhận n và tên phép đo từ dòng lệnh.',
    needs: [
      'Cách viết hệ thức truy hồi và cách giải nó bằng lối đếm theo tầng — phần Lý thuyết bài này.',
      'Đệ quy có ghi nhớ khác đệ quy trần ở chỗ nào, và vì sao khác biệt đó lớn tới vậy.',
      'C++: hàm đệ quy, một mảng làm bộ nhớ ghi nhớ, và kiểu long long để khỏi tràn số từ n = 47.',
      'Công cụ bench ở bài Độ phức tạp thuật toán, dùng ở yêu cầu cuối để vẽ biểu đồ.',
    ],
    output: 'Một bảng số lời gọi theo n cho ba cách cài fib, cộng một biểu đồ ba đường. Chú ý đây là số lời gọi chứ không phải mili giây — con số này không đổi dù bạn chạy trên máy nào.',
    outputSample: `$ ./bench_dequy fib 5,10,15,20,25

  n | de quy tran | ghi nho | vong lap
  5 |          15 |       9 |        5
 10 |         177 |      19 |       10
 15 |        1973 |      29 |       15
 20 |       21891 |      39 |       20
...
de quy tran khop cong thuc 2*fib(n+1) - 1`,
    start: [
      'Viết fib đệ quy trần đúng ba dòng. Thêm một biến đếm toàn cục, tăng nó ở dòng đầu của hàm. Chạy fib(10) rồi in biến đếm ra.',
      'Đặt biến đếm về 0 trước mỗi lần đo. Đây đúng là cái bẫy ghi ở cuối trang — gặp nó một lần rồi sẽ nhớ.',
      'Cho n chạy từ 5 tới 25, in bảng hai cột n và số lời gọi. Đối chiếu với công thức 2·fib(n+1) − 1 xem có khớp không.',
      'Viết fib có ghi nhớ, đếm lại. Nhìn con số tụt từ hàng trăm nghìn xuống vài chục — đó là toàn bộ bài học của dự án này.',
      'Viết fib vòng lặp, thêm cột thứ ba vào bảng.',
      'Cuối cùng mới tổng quát hoá bộ đếm thành thứ dùng được cho hàm đệ quy bất kỳ, rồi vẽ ba đường bằng bench.',
    ],
    must: [
      'Cài bộ đếm lời gọi dùng được cho hàm đệ quy bất kỳ, không chỉ riêng fib.',
      'Cài fib ba cách — đệ quy trần, đệ quy có ghi nhớ, vòng lặp — và in bảng số lời gọi theo n.',
      'Từ số liệu đo được, tự suy ra hệ thức truy hồi và so với hệ thức viết bằng tay.',
      'Đo bằng bench của thư viện core bạn viết ở bài trước, vẽ số lời gọi theo n cho cả ba cách trên cùng biểu đồ và giải thích vì sao đường của bản đệ quy trần bốc lên thẳng đứng.',
    ],
    done: [
      {
        dat: 'Số lời gọi của fib đệ quy trần khớp công thức lý thuyết.',
        kiem: 'Với n = 10, 15, 20, bảng in ra 177, 1973, 21891 — đúng bằng 2·fib(n+1) − 1. Lệch một đơn vị nghĩa là bạn đếm sai chỗ đặt biến đếm.',
      },
      {
        dat: 'Bộ đếm được đặt lại đúng giữa hai phép đo.',
        kiem: 'Đo n = 10 hai lần liên tiếp trong cùng một lần chạy chương trình: hai lần phải ra cùng một con số.',
      },
      {
        dat: 'Bản ghi nhớ có số lời gọi tuyến tính theo n.',
        kiem: 'Nhìn cột ghi nhớ trong bảng: hiệu giữa hai hàng liên tiếp là một hằng số, không phải nhân lên.',
      },
      {
        dat: 'Bản vòng lặp không tràn số ở n lớn.',
        kiem: 'Chạy fib vòng lặp với n = 90 và đối chiếu với giá trị tra được. Ra số âm nghĩa là bạn còn dùng int thay vì long long.',
      },
      {
        dat: 'Bộ đếm dùng được cho hàm đệ quy khác, không chỉ fib.',
        kiem: 'Gắn nó vào một hàm đệ quy thứ hai bất kỳ — ví dụ giai thừa hoặc tìm nhị phân đệ quy — mà không phải sửa mã của bộ đếm.',
      },
      {
        dat: 'Ba ca kiểm tự động đều xanh.',
        kiem: 'make test — in 3/3 ca xanh, trong đó có ca đối chiếu số lời gọi với công thức 2·fib(n+1) − 1.',
      },
    ],
    traps: [
      'Đếm lời gọi bằng biến toàn cục rồi quên đặt lại giữa hai phép đo.',
      'Để n quá lớn ở bản đệ quy trần rồi tưởng chương trình treo.',
      'Đo fib bản vòng lặp bằng int rồi tràn số từ n = 47.',
      'Ghi nhớ bằng mảng nhưng không phân biệt "chưa tính" với "kết quả bằng 0".',
      'Quên rằng độ sâu đệ quy cũng là bộ nhớ, không chỉ thời gian.',
    ],
  },
}
