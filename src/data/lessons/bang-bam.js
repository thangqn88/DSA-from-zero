export default {
  ...
  project: {
    ...
    why: 'Đây là bước đầu tiên của mọi công cụ phân tích văn bản: thống kê từ khoá, lọc thư rác, dựng từ điển cho ô tìm kiếm. Ở Chương 3 bạn sẽ dùng lại chính bộ đếm này để xây từ điển cho máy gợi ý tìm kiếm, nên hãy viết nó thành một module dùng lại được chứ đừng nhét hết vào hàm main.',
    input: 'Một file .txt bất kỳ, ví dụ một quyển sách tải từ Project Gutenberg tại https://www.gutenberg.org. File càng lớn thì khác biệt giữa các cách cài càng rõ.',
    needs: [
      'Hàm băm, va chạm, chuỗi móc nối và hệ số tải — trọn phần Lý thuyết bài này.',
      'Vì sao nở bảng bắt buộc phải băm lại toàn bộ chứ không sao chép nguyên vị trí cũ. Đây là chỗ sai kinh điển, và ví dụ thứ hai của bài đã mổ xẻ nó.',
      'C++: đọc file bằng ifstream, tách từ bằng istringstream, con trỏ để nối chuỗi móc nối.',
      'Vec và Str từ Chương 1 nếu bạn muốn dùng lại. Dùng std::string cũng được — thứ bị cấm chỉ là std::unordered_map và std::map.',
      'Công cụ đo ở bài Độ phức tạp thuật toán, dùng ở yêu cầu cuối.',
    ],
    output: 'Một chương trình nhận đường dẫn file văn bản, in ra 20 từ hay gặp nhất kèm số lần, và một bảng phân bố số phần tử theo ngăn để bạn tự chấm hàm băm của mình rải có đều không.',
    outputSample: `$ ./demtu sach.txt
Doc 1043211 tu, 24187 tu khac nhau
Bang 65536 ngan, he so tai 0.37, da no 12 lan
...
Phan bo ngan: trong 41288 | 1 phan tu 18455 | 2-3 phan tu 5210 | tren 3 la 583
Chuoi dai nhat: 6 -> ham bam rai deu, dat`,
    start: [
      'Đọc file và in ra số từ đếm được. Chưa băm gì cả — chỉ cần chắc rằng bạn tách từ đúng và bỏ được dấu câu.',
      'Cài bảng băm cố định 16 ngăn, chưa nở. Hàm băm dùng h = h*31 + c. Thêm được và tra được là đủ.',
      'Kiểm va chạm bằng tay: cố tình thêm hai từ mà bạn biết chúng rơi cùng ngăn, rồi tra lại từng từ xem có ra đúng giá trị riêng của nó không.',
      'Thêm hệ số tải và phần nở bảng gấp đôi. In số ngăn sau mỗi lần nở để nhìn nó nhảy 16, 32, 64.',
      'Chạy trên file thật, in 20 từ hay gặp nhất. Đối chiếu với một bản viết bằng std::unordered_map để chắc kết quả đúng trước khi đi tiếp.',
      'Cuối cùng mới in phân bố ngăn và đo so với cách duyệt tuyến tính một mảng cặp khoá-giá trị.',
    ],
    must: [
      'Đọc file, tách thành từ, chuyển hết về chữ thường và bỏ dấu câu.',
      'Tự cài bảng băm ánh xạ chuỗi sang số đếm, xử lý va chạm bằng chuỗi móc nối. Không dùng std::unordered_map hay std::map.',
      ...
      {
        dat: 'Chạy hết một file khoảng 1MB trong dưới một giây.',
        kiem: 'time ./demtu sach.txt — cột real dưới 1s. Lâu hơn nhiều nghĩa là hàm băm dồn cục hoặc bảng chưa nở.',
      },
      {
        dat: 'Kết quả đúng, đối chiếu được với một bản mốc.',
        kiem: 'Viết thêm một bản 10 dòng dùng std::unordered_map, chạy trên cùng file, so 20 từ đầu: trùng khớp cả từ lẫn số đếm.',
      },
      {
        dat: 'Dữ liệu còn nguyên sau khi bảng nở.',
        kiem: 'Thêm 1000 khoá (bảng sẽ nở nhiều lần), rồi tra lại đủ 1000 khoá đó: không khoá nào mất, không giá trị nào sai. Đây là ca bắt lỗi quên băm lại.',
      },
      {
        dat: 'Hai khoá va chạm vẫn lấy đúng giá trị riêng.',
        kiem: 'Cố tình thêm hai từ mà bạn tính được là cùng ngăn, gán hai giá trị khác nhau, tra lại từng từ: ra đúng giá trị của chính nó.',
      },
      {
        dat: 'Hàm băm rải đều, tự chấm được bằng số.',
        kiem: 'Nhìn bảng phân bố ngăn in ra: chuỗi dài nhất không quá 10 khi hệ số tải dưới 0,75. Dài hơn nhiều nghĩa là hàm băm còn yếu.',
      },
      {
        dat: 'Không rò rỉ bộ nhớ ở các nút của chuỗi móc nối.',
        kiem: 'Chạy dưới -fsanitize=address trên file 1MB — báo 0 byte rò rỉ, kể cả sau nhiều lần nở bảng.',
      },
      {
        dat: 'Đo được khoảng cách với cách duyệt tuyến tính.',
        kiem: 'Trên cùng bộ từ, tra cứu bằng bảng băm nhanh hơn duyệt mảng cặp khoá-giá trị ít nhất 100 lần. Viết một câu giải thích bằng ngôn ngữ O lớn (Big O).',
      },
    ],
    traps: [
      'Hàm băm quá yếu khiến mọi khoá rơi vào vài ngăn — bảng băm biến thành danh sách liên kết và O(1) thành O(n).',
      'Quên băm lại toàn bộ khi nở bảng, chỉ sao chép nguyên vị trí cũ sang bảng mới.',
      'Rò rỉ bộ nhớ ở các nút của chuỗi móc nối khi xoá hoặc khi nở bảng.',
      'Tràn số khi tính mã băm đa thức mà không lấy dư dần.',
      'Coi hai khoá cùng mã băm là cùng một khoá — vẫn phải so khoá thật sau khi tìm đúng ngăn.',
    ],
  },
}
