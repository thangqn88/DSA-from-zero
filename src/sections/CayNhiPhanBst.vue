...
<h2>Cây Nhị Phân — 3 Phép Duyệt, Cha-Con, BST <span class="exam-tag">★ Đề ôn tập — Bài 7, 8 · Đề bổ sung — D05006, D05008, D05001</span></h2>
<div class="mini-toc">
  <span class="mt-label">Chuyển nhanh tới</span>
  <a class="mt-exam" href="#bai7-8-bst-build-delete">★ Bài 7&amp;8 — Xây BST, duyệt, xóa</a>
  <a href="#bai-cay-cha-con">Dựng cây từ cặp cha-con</a>
  <a class="mt-exam" href="#cay-day-du-bst-postorder-muc">D05006, D05008, D05001 — Cây đầy đủ, Preorder→Postorder, Duyệt mức</a>
</div>

<h3 id="auto-cay-nhi-phan-va-bst">Cây nhị phân và BST</h3>
<p class="idea-label">🧩 Ý tưởng cốt lõi</p>
<p>Cây gia phả: ông bà (gốc) → tối đa 2 con (trái, phải) → mỗi con lại có tối đa 2 con riêng. <strong>BST</strong>: mọi node, toàn bộ cây con trái nhỏ hơn nó, toàn bộ cây con phải lớn hơn — giống tìm từ trong từ điển.</p>

<h3 id="auto-3-phep-duyet-cay">3 phép duyệt cây</h3>
<p class="idea-label">🧩 Ý tưởng cốt lõi</p>
<p><strong>NLR</strong> (tiền tự): Gốc→Trái→Phải. <strong>LNR</strong> (trung tự): Trái→Gốc→Phải — với BST luôn ra dãy TĂNG DẦN. <strong>LRN</strong> (hậu tự): Trái→Phải→Gốc.</p>
...
<blockquote><p>Mẹo nhớ: chỉ cần nhớ vị trí chữ N (Gốc) nằm ở đâu trong tên gọi. Code của 3 hàm giống hệt nhau, chỉ đổi vị trí dòng <code>cout</code>.</p></blockquote>
...
<h3 id="auto-chen-gia-tri-vao-bst-nen-tang-cho-bai-chinh-thuc-b">Chèn giá trị vào BST (nền tảng cho bài chính thức bên dưới)</h3>
<pre v-pre><code>#include &lt;bits/stdc++.h&gt;
using namespace std;
...
<div class="realworld">
  ...
  <dl>
    <dt>Dùng ở đâu</dt>
    <dd>BST là nền tảng của index trong cơ sở dữ liệu (B-Tree/B+Tree trong MySQL, PostgreSQL); cấu trúc <code>std::map</code>/<code>std::set</code> trong C++ dùng Red-Black Tree (1 biến thể BST tự cân bằng); thao tác Undo/Redo phân cấp trong phần mềm đồ họa.</dd>
    ...
    <dd class="bad">Nếu dữ liệu chèn vào theo thứ tự đã sắp sẵn — BST thường (không tự cân bằng) suy biến thành danh sách liên kết, chậm còn O(n). Cần dùng AVL/Red-Black Tree để đảm bảo cân bằng.</dd>
  </dl>
</div>

<h3 id="bai7-8-bst-build-delete">★ Bài chính thức trong Đề ôn tập — Bài 7 &amp; 8: Xây BST, in Post Order, và Xóa một khóa bằng phương pháp copy</h3>
...
<p style="margin-top:0.5rem;"><em>Lưu ý: dù phần Input mô tả ở trên chỉ ghi "n rồi tới mảng", ví dụ thực tế có thêm số T ở dòng đầu (số bộ test) — theo đúng ví dụ để code khớp định dạng.</em></p>
</div>

<div class="problem-box">
<span class="pb-title">📋 Nguyên văn đề bài 8 — BST đầy đủ: cài đặt, chèn, xây cây, duyệt, xóa (7 điểm)</span>
<p>Giả sử ta có cây nhị phân tìm kiếm <em>tree</em> có khóa là một số nguyên.</p>
<ol>
  ...
  <li>Tạo cây nhị phân tìm kiếm bằng cách thêm dần n số nguyên vào cây nhị phân tìm kiếm (n số nguyên nhập từ bàn phím) (1 điểm) (1≤n≤10³)</li>
  <li>In cây theo thứ tự duyệt sau (1 điểm)</li>
  <li>Viết phép toán xóa một khóa x trên cây nhị phân tìm kiếm bằng phương pháp copy (2 điểm) (x nhập từ bàn phím, 1≤x≤10³)</li>
  <li>In cây sau khi xóa theo thứ tự duyệt trước (1 điểm)</li>
</ol>
<p><strong>Input:</strong> Dòng đầu tiên chứa số nguyên dương n là số phần tử trên cây (1≤n≤10³). Dòng 2 chứa n số nguyên (1≤aᵢ≤10³). Dòng 3 là số nguyên cần xóa trên cây.</p>
<p><strong>Output:</strong> gồm 2 dòng — Dòng 1 là thứ tự duyệt cây theo thứ tự sau của cây tạo ở ý c. Dòng 2 là thứ tự duyệt cây theo thứ tự trước của cây sau khi xóa ở ý e.</p>
<table class="formula-table"><tr><th>Input</th><th>Output</th></tr>
<tr><td>5<br>4 2 1 6 3<br>1</td><td>1 3 2 6 4<br>4 2 3 6</td></tr></table>
</div>

<blockquote><p>📌 Phần duyệt cây/BST ở trên là nền tảng bắt buộc. <strong>Đây mới là bài bạn sẽ gặp đúng nguyên văn trong đề thi</strong> — bao gồm cả phần khó nhất: xóa node có 2 con.</p></blockquote>

<blockquote><p>⚠️ <strong>Lưu ý quan trọng</strong>: Bài 7 và Bài 8 tuy dùng chung 1 kiến thức (xây BST + Post Order) nhưng có <strong>định dạng input/output KHÁC NHAU</strong> — đọc kỹ để không mất điểm oan:</p>
<ul>
  <li><strong>Bài 7</strong> có nhiều bộ test (đọc số T ở đầu), và output phải in kèm tiền tố <code>"Test #1: "</code>, <code>"Test #2: "</code>... trước kết quả mỗi test.</li>
  <li><strong>Bài 8</strong> chỉ có <strong>1 bộ test duy nhất</strong> (không có T), không cần tiền tố "Test #", và có thêm yêu cầu xóa 1 khóa.</li>
</ul></blockquote>

<p class="idea-label">🧩 Ý tưởng cốt lõi — Bài 7: xây cây rồi in Post Order (nhiều bộ test)</p>
<p>Với mỗi bộ test, xây 1 cây BST mới từ đầu bằng cách chèn lần lượt các phần tử của mảng — <strong>không sắp xếp lại mảng trước khi chèn</strong>, vì thứ tự chèn quyết định hình dạng cây. Sau đó in Post Order kèm tiền tố "Test #i:".</p>
...
<p><strong>Chạy tay đúng ví dụ trong đề (Bài 7)</strong>: T=3 bộ test.</p>
<table class="formula-table">
  <tr><th>Test</th><th>Mảng chèn</th><th>Post Order kết quả</th></tr>
  <tr><td>#1</td><td>{3, 1, 4, 2, 5}</td><td>2 1 5 4 3</td></tr>
  <tr><td>#2</td><td>{1, 6, 2, 3, 8, 5, 7, 4}</td><td>4 5 3 2 7 8 6 1</td></tr>
  <tr><td>#3</td><td>{4, 7, 2, 5, 3, 6, 1}</td><td>1 3 6 2 5 7 4</td></tr>
</table>
<p><strong>Kiểm chứng nhanh Test #1</strong> (tự chạy tay để tin tưởng công thức): chèn 3,1,4,2,5 → gốc 3 (trái 1 (phải 2), phải 4 (phải 5)). Post Order: nhánh trái của 3 trước (1's subtree: đi trái=null, phải=2, rồi 1 → "2 1"), rồi nhánh phải của 3 (4's subtree: trái=null,phải=5,rồi 4 → "5 4"), cuối cùng gốc 3 → ghép lại: <strong>2 1 5 4 3</strong> ✓ khớp đúng đề.</p>
<p>Output đầy đủ đúng định dạng: <code>Test #1: 2 1 5 4 3</code> / <code>Test #2: 4 5 3 2 7 8 6 1</code> / <code>Test #3: 1 3 6 2 5 7 4</code>.</p>

<p class="idea-label" style="margin-top:2rem;">🧩 Ý tưởng cốt lõi — Bài 8: xây cây, xóa 1 khóa bằng phương pháp copy (chỉ 1 bộ test)</p>
<p>Bài 8 dùng lại đúng <code>insertBST</code> và <code>postorder</code> ở trên, nhưng KHÔNG có nhiều bộ test T, và output KHÔNG có tiền tố "Test #". Ví dụ dưới đây là ví dụ ĐÚNG và DUY NHẤT của riêng Bài 8 trong đề — khác với ví dụ Bài 7 ở trên.</p>

<p><strong>Chạy tay đúng ví dụ trong đề (Bài 8)</strong>: mảng a = {4, 2, 1, 6, 3}, chèn lần lượt:</p>
<table class="formula-table">
  <tr><th>Chèn</th><th>Việc làm</th><th>Cây sau khi chèn</th></tr>
  <tr><td>4</td><td>Cây rỗng → 4 làm gốc</td><td>gốc = 4</td></tr>
  <tr><td>2</td><td>2 &lt; 4 → sang trái của 4</td><td>4 (trái: 2)</td></tr>
  <tr><td>1</td><td>1 &lt; 4 → trái (2); 1 &lt; 2 → trái của 2</td><td>4 (trái: 2 (trái: 1))</td></tr>
  <tr><td>6</td><td>6 &gt; 4 → sang phải của 4</td><td>+ phải của 4 = 6</td></tr>
  <tr><td>3</td><td>3 &lt; 4 → trái (2); 3 &gt; 2 → phải của 2</td><td>+ phải của 2 = 3</td></tr>
</table>
...
<p>Post Order = Trái → Phải → Gốc: duyệt(2's subtree) trước, tức 1, 3, 2, rồi tới subtree của 6 (chỉ có 6), cuối cùng mới tới gốc 4:</p>
<p style="text-align:center; font-family:monospace; font-size:1.1rem;">Post Order = <strong>1 3 2 6 4</strong> ✓ khớp đúng đề (dòng 1 của output Bài 8)</p>
<p class="idea-label" style="margin-top:1.5rem;">🧩 Cách xóa 1 khóa bằng phương pháp copy (ý e của Bài 8)</p>
<p>Xóa 1 node trong BST khó hơn chèn vì phải giữ đúng tính chất BST sau khi xóa. Hãy hình dung bạn là quản lý và muốn cho 1 nhân viên (node) nghỉ việc — có 3 tình huống:</p>
<ol>
  <li><strong>Nhân viên đó không quản lý ai (lá — không con)</strong>: đơn giản, cho nghỉ luôn, xóa thẳng.</li>
  <li><strong>Nhân viên đó chỉ có 1 cấp dưới (1 con)</strong>: cấp dưới đó "lên thay" ngay vị trí của người vừa nghỉ.</li>
  <li><strong>Nhân viên đó có 2 cấp dưới (2 con)</strong> — phức tạp nhất: không thể để cả 2 cấp dưới cùng "lên thay". Ta tìm 1 người phù hợp nhất để thế chỗ mà <strong>không phá vỡ trật tự BST</strong>: đó chính là <strong>người kế tiếp theo thứ tự (inorder successor)</strong> — giá trị <strong>nhỏ nhất bên nhánh phải</strong> (đi thẳng sang phải 1 bước rồi đi mãi sang trái). Ta <strong>copy giá trị</strong> của người này lên node cần xóa (đây là lý do gọi là "phương pháp copy"), rồi xóa người kế tiếp đó ở vị trí cũ của họ — mà người kế tiếp chắc chắn chỉ có tối đa 1 con (vì nó là nhỏ nhất, không thể có con trái), nên việc xóa nó lại quay về tình huống (1) hoặc (2), đơn giản hơn.</li>
</ol>
...
<blockquote><p>📎 Struct <code>Node</code> dùng con trỏ (<code>Node *left, *right</code>) đúng theo yêu cầu câu a của Bài 8 ("cài đặt BST bằng con trỏ"); <code>insertBST</code> đúng câu b, c; <code>postorder</code> đúng câu d; <code>deleteNode</code> đúng câu e; <code>preorder</code> sau khi xóa đúng câu f.</p></blockquote>

<p><strong>Chạy tay đúng ví dụ trong đề (Bài 8)</strong>: n=5, mảng {4,2,1,6,3} (cây giống hệt Bài 7 ở trên), xóa x=1.</p>
<table class="formula-table">
  <tr><th>Bước</th><th>Việc làm</th></tr>
  <tr><td>Tìm node 1</td><td>1 &lt; 4 → sang trái (2); 1 &lt; 2 → sang trái (1) → tìm thấy</td></tr>
  <tr><td>Node 1 là loại nào?</td><td>Không có con trái, không có con phải → <strong>Trường hợp 1 (lá)</strong> — xóa thẳng</td></tr>
  <tr><td>Cây sau khi xóa</td><td>4 (trái: 2 (chỉ còn phải: 3), phải: 6)</td></tr>
</table>
<p style="text-align:center; font-family:monospace; font-size:1.1rem;">Preorder sau khi xóa (Gốc→Trái→Phải) = <strong>4 2 3 6</strong> ✓ khớp đúng đề</p>

<blockquote><p>💡 <strong>Để hiểu sâu hơn trường hợp 3 (2 con)</strong> — tự luyện thêm: nếu xóa node <strong>2</strong> (có đủ 2 con: 1 và 3) khỏi cây gốc ở Bài 7, ta tìm <code>findMin</code> của nhánh phải node 2 (chỉ có mỗi node 3) → successor = 3. Copy giá trị 3 lên vị trí node 2, rồi xóa node 3 ở vị trí cũ (nó là lá, quay về Trường hợp 1). Cây mới: gốc 4 (trái: 3 (trái: 1), phải: 6) — chú ý node 1 vẫn còn nguyên, chỉ có "nhãn" ở gốc nhánh trái đổi từ 2 thành 3.</p></blockquote>

<h3 id="auto-4-tinh-chat-hay-hoi">4 tính chất hay hỏi</h3>
<p class="idea-label">🧩 Ý tưởng cốt lõi</p>
<pre v-pre><code>#include &lt;bits/stdc++.h&gt;
using namespace std;
...
<blockquote><p>🎯 Nhận ra khuôn chung: cả 4 hàm đi theo cấu trúc "nếu cây rỗng trả về giá trị cơ sở; nếu không, xử lý gốc rồi kết hợp kết quả đệ quy của 2 nhánh con" — giống hệt hàm <code>height()</code>. Đây là khuôn đệ quy tổng quát cho hầu hết bài toán trên cây.</p></blockquote>

<h4 id="auto-luyen-tap">Luyện tập</h4>
<ol class="practice">
  <li>Chèn 10,5,15,3,7,12,18 vào BST; viết cả 3 kết quả duyệt và toàn bộ root-to-leaf paths.
    <div class="idea"><em>Ý tưởng:</em> mỗi số mới chỉ cần so sánh với gốc hiện tại để quyết định "rẽ trái hay rẽ phải", rồi lặp lại y hệt ở node con — đây chính là "tìm kiếm nhị phân" áp dụng lúc chèn.</div>
    <div class="hint"><em>Hướng dẫn:</em> gốc 10, trái 5 (con 3,7), phải 15 (con 12,18). LNR phải ra 3,5,7,10,12,15,18.</div>
  </li>
  <li>Vẽ tay cây sau khi chạy <code>mirrorTree()</code> trên cây ở widget trên.
    <div class="idea"><em>Ý tưởng:</em> phản chiếu không chỉ đổi chỗ 2 con của gốc — nó phải áp dụng ở MỌI cấp, kể cả các cháu/chắt. Vẽ từ lá lên sẽ tránh bỏ sót cấp nào.</div>
  </li>
  <li>Viết hàm đếm tổng số node và hàm tìm giá trị lớn nhất trong BST.
    <div class="idea"><em>Ý tưởng:</em> đếm node dùng đúng khuôn đệ quy "1 (chính nó) + kết quả nhánh trái + kết quả nhánh phải" — giống hệt <code>countLeaves</code> nhưng không cần điều kiện "là lá". Tìm giá trị lớn nhất trong BST thì không cần đệ quy — chỉ cần tận dụng tính chất sắp xếp, đi thẳng một đường sang phải.</div>
  </li>
  <li>(Bài 7&amp;8 trong đề) Tự xây tay BST từ mảng {8, 3, 10, 1, 6, 14, 4, 7, 13}, viết Post Order, rồi xóa khóa 3 (có 2 con) và viết lại Pre Order sau khi xóa.
    <div class="idea"><em>Ý tưởng:</em> đây là trường hợp 3 con đầy đủ (2 con) — trước khi xóa, tự xác định trước "ai sẽ là inorder successor của node 3" (tức giá trị nhỏ nhất bên nhánh phải của 3) rồi mới bắt đầu xóa, để không bị rối khi code chạy qua nhiều bước đệ quy.</div>
    <div class="hint"><em>Hướng dẫn:</em> cây ban đầu: gốc 8 (trái 3 (con 1, 6 (con 4,7)), phải 10 (con 14 (con trái 13))). Successor của 3 là 4 (nhỏ nhất bên phải của 3). Sau khi xóa: gốc 8 (trái 4 (con 1, 6 (chỉ còn phải 7)), phải 10...).</div>
  </li>
</ol>
...
<ol class="practice">
  <li>Tự vẽ cây từ ví dụ 2 của D05001 (10 20 L, 10 30 R, 20 40 L, 20 60 R), tự chạy tay level-order trước khi đối chiếu — đáp án đúng là "10 20 30 40 60".
    <div class="idea"><em>Ý tưởng:</em> mức 0 chỉ có gốc; mức 1 là toàn bộ con của gốc theo đúng thứ tự trái→phải; mức 2 là toàn bộ con của các node ở mức 1, cũng duyệt trái→phải theo đúng thứ tự cha của chúng xuất hiện ở mức 1.</div>
  </li>
  <li>Với D05008, thử preorder {50, 30, 20, 40, 70, 60, 80} — tự chèn tay vào BST rỗng rồi viết ra Postorder.
    <div class="hint"><em>Hướng dẫn:</em> cây: 50(trái 30(trái 20, phải 40), phải 70(trái 60, phải 80)). Postorder: 20 40 30 60 80 70 50.</div>
  </li>
</ol>
